...
public:
  ...
  using ServiceUpdateATRPathList = rclcpp::Service<atr_interfaces::srv::UpdateATRPathList>::SharedPtr;
  ...
  using SrvRespFutureUpdateATRPathList = rclcpp::Client<atr_interfaces::srv::UpdateATRPathList>::SharedFuture;
   ...
   using ATRUpdatedPathListShPt = atr_interfaces::msg::ATRPathList::SharedPtr;
...
private:
  ...
  ServiceUpdateATRPathList path_service_;         ///< Client to update ATR path list from scheduler
  ...
  std::string service_path_scheduler_;  ///< name for the service to update the ATR Path List from scheduler
  ...
  std::mutex atr_path_mutex_;  ///< mutex to protect write/read access between publisher and client
   ...
   bool data_ready_; ///< boolean indicating if path from scheduler available
   atr_interfaces::msg::ATRPathList local_scheduler_path_; ///< local variable containing received path from scheduler
...
private:
  ...
  /**
   * @brief update path list received from scheduler
   *
   * @param request new path list
   * @param response success flag with error message describing the failure
   */
  void updatePathlistCB(const std::shared_ptr<atr_interfaces::srv::UpdateATRPathList::Request> request,
