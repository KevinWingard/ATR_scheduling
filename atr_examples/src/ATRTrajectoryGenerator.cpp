...
ATRTrajectoryGenerator::ATRTrajectoryGenerator() : Node("object_list_subscriber")
{
  ...
  // received_atr_path_list_ = std::make_shared<atr_interfaces::msg::ATRPathList>();
  ...
  // atr_interfaces::msg::ATRPathList local_scheduler_path_;
  ...
  // Update Local Predicted Object List
  path_service_ = create_service<atr_interfaces::srv::UpdateATRPathList>(
      service_path_scheduler_, std::bind(&ATRTrajectoryGenerator::updatePathlistCB, this, _1, _2));
...
void ATRTrajectoryGenerator::init()
{
  ...
  service_path_scheduler_ = params.at(9).as_string();

  data_ready_=false;
}

void ATRTrajectoryGenerator::updatePathlistCB(
...
{
   RCLCPP_INFO_STREAM(get_logger(), "got path from scheduler");
  local_scheduler_path_=request->list;
...
void ATRTrajectoryGenerator::timer_callback()
{
  ...
  // if (init_phase){
// Create init phase such that ATRs are stationary at start positions until first data received
  // }
  ...
  if (data_ready_)
  {
    ...
    // auto local_scheduler_path_ = std::make_shared<atr_interfaces::msg::ATRPathList>();
    ...
    Eigen::MatrixXd Points = Eigen::MatrixXd::Zero(6, poses_size);
    ...
    Points.row(4) << -0.5, -0.5, -1.0, -1.0, -1.0, -0.5, -0.5, -0.5, -0.5, -0.5, -0.7, -0.9, -1.1, -1.3;
    Points.row(5) << 1, 0.7, 0.4, 0.1, -0.2, -0.5, -0.8, -1.1, -1.4, -1.7, -1.7, -1.7, -1.7, -1.7;
    ...
    atr_path_list_->paths = local_scheduler_path_.paths;
    ...
    if (atr_path_list_client_->service_is_ready())
    {
      ...
      request->list = local_scheduler_path_;
