...
void NonaGenerator::timer_callback()
{
  ...
  // Simulated Areas
  // Non accessible areas (nonaa)
  // double forklift_area[]={4.6,4.8,0,0.6,4.8,0,0.6,2.8,0,11.1,2.8,0,11.1,4.8,0,5.6,4.8,0};

  // // outline
  // v_Points.push_back(Eigen::MatrixXd(12, 3));
  // v_Points[0] << 4.6,4.8,0,0.6,4.8,0,0.6,2.8,0,11.1,2.8,0,11.1,4.8,0,5.6,4.8,0,5.6,5.3,0,11.1,5.3,0,11.1,10.2,0,0.6,10.2,0,0.6,5.3,0,4.6,5.3,0;
  ...
  // v_Points[1] << 3.6,6.7,0,4.6,6.7,0,4.6,7.3,0,3.6,7.3,0;
  // v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  // v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  // // rectangular box
  // v_Points.push_back(Eigen::MatrixXd(4, 3));
  // v_Points[2] << 5.6,6.7,0,8.6,6.7,0,8.6,7.3,0,5.6,7.3,0;
  // v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  // v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  //  // Table
  // v_Points.push_back(Eigen::MatrixXd(4, 3));
  // v_Points[3] << 2.25,9.75,0,5.35,9.75,0,5.35,10.65,0,2.25,10.65,0;
  // v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  // v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  //    // RITA
  // v_Points.push_back(Eigen::MatrixXd(4, 3));
  // v_Points[4] << 5.4,8.55,0,9.9,8.55,0,9.9,10.2,0,5.4,10.2,0;
  // v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  // v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  //      // innerline
  // v_Points.push_back(Eigen::MatrixXd(8, 3));
  // v_Points[5] << 5.6,5.3,0,9.6,5.3,0,9.6,8.3,0,5.15,8.3,0,5.15,9.3,0,2.6,9.3,0,2.6,5.3,0,4.6,5.3,0;
  // v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  // v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  // The coordinate system is following the coord. sys. of the drawing of the plant except that the y-dimension is inverted.
  // Horseshoe in middle of plant. Non accessible area (nonaa)
  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[0] << 2.490, -4.586, 0, 3.490, -4.568, 0, 3.490, -4.536, 0, 2.490, -4.536, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);
  
  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[1] << 2.490, -5.726, 0, 2.540, -5.726, 0, 2.540, -4.586, 0, 2.490, -4.586, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[2] << 2.490, -5.776, 0, 3.490, -5.776, 0, 3.490, -5.726, 0, 2.490, -5.726, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  // Rectangle in middle of plant. Non accessible area (nonaa)

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[3] << 4.490, -5.776, 0, 8.990, -5.776, 0, 8.990, -4.536, 0, 4.490, -4.536, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  // Bar above horseshoe in plant. Non accessible area (nonaa)

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[4] << 2.490, -3.476, 0, 3.490, -3.476, 0, 3.490, -3.426, 0, 2.490, -3.426, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);
  ...
  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[5] << 1.350, -7.234, 0, 9.016, -7.234, 0, 9.016,-7.184, 0, 1.350, -7.184, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[6] << 1.350, -7.184, 0, 1.400, -7.184, 0, 1.400, -0.475, 0, 1.350, -0.475, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);
  ...
  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[7] << 2.490, -2.427, 0, 4.624,-2.427, 0, 4.624, -2.377, 0, 2.490, -2.377, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[8] << 4.574, -2.427, 0, 4.574, -3.339, 0, 4.624, -3.339, 0, 4.624, -2.427, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[9] << 4.574, -3.389, 0, 8.974, -3.389, 0, 8.974, -3.339, 0, 4.574, -3.339, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  // Fixed obstacle bottom left

  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[10] << 9.242, -7.484, 0, 10.392, -7.484, 0, 10.392, -6.724, 0, 9.242, -6.724, 0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);

  // Outline
  v_Points.push_back(Eigen::MatrixXd(4, 3));
  v_Points[11] << 0,-0.5,0,0,-7.5,0,12.2,-7.5,0,12.2,-0.5,0;
  v_classes.push_back(atr_interfaces::msg::ObjectClass::WALL);
  v_types.push_back(atr_interfaces::msg::ObjectType::NONAA);
  ...
  {
    if (i==11) {aux_marker.color.g=0.0f;}
    else {aux_marker.color.g=1.0f;}
