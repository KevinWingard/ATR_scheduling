...
def Compositional(problem,ws_path):
    ...
    jobs, nodes, edges, Autonomy, ATRs, charging_coefficient = json_parser('%s/test_cases/%s.json' % (ws_path,problem))

    # print some info
    print(f"\nNumber of jobs: {len(jobs)}")
    print("ATR types and quantities:")
    for atr in ATRs.items():
        print(atr)
    print("\n")
    ...
    return node_sequence,edge_sequence,instance,optimum,running_time,len(previous_routes),
