...
def routing(edges,jobs,tasks,Autonomy,current_path,previous_routes = []):
    ...
    domain_1 = m.addConstrs(
        autonomy_left[i,j] >= 0 for i in vehicles for j in tasks
    )

    domain_2 = m.addConstrs(
        autonomy_left[i, j] <= Autonomy for i in vehicles for j in tasks
    )

    domain_3 = m.addConstrs(
        customer_served[j] >= 0 for j in tasks
    )
    ...
    infer_arrival_time = m.addConstrs(
        ...
        (customer_served[j] >= customer_served[i] +
                                distances[(i,j)] +
                                jobs['job' + '_' + i.split('_')[1]]['tasks'][i.split('_')[2]]['Service'])
        for k in vehicles
        for i in tasks
        for j in tasks
        if i.split('_')[0] != 'start'
    ...
    time_window_1 = m.addConstrs(
        customer_served[j] >= jobs['job' + '_' + j.split('_')[1]]['tasks'][j.split('_')[2]]['TW'][0]
        for j in tasks
        if j.split('_')[0] != 'start'
        and j.split('_')[0] != 'end'
        and jobs['job' + '_' + j.split('_')[1]]['tasks'][j.split('_')[2]]['TW'] != 'None'
    )

    time_window_2 = m.addConstrs(
        customer_served[j] <= jobs['job' + '_' + j.split('_')[1]]['tasks'][j.split('_')[2]]['TW'][1]
        for j in tasks
        if j.split('_')[0] != 'start'
        and j.split('_')[0] != 'end'
        and jobs['job' + '_' + j.split('_')[1]]['tasks'][j.split('_')[2]]['TW'] != 'None'
    ...
    # may not need this with three indexes
    # route_continuity = [
    #
    ...
    routing_feasibility = m.feasibility()
